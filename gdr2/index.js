...
const GDR_URL = 'https://drive.google.com/file/d/1nXjm6xDhWX_TkTD-vrElYGzpKk5NBw6m/view?usp=drivesdk';
const CHUNK_SIZE = 10 * 1024 * 1024; // 10 MB
const CHUNKS_DIR = 'chunks';

const maxChunksToDownload = process.argv[2] ? parseInt(process.argv[2], 10) : null;
...
async function getDownloadUrlAndCookies() {
    ...
    let intermediateUrlMatch = initialResponse.data.match(/"(https?:\/\/drive\.usercontent\.google\.com\/uc\?[^"]+)"/);
    if (!intermediateUrlMatch || !intermediateUrlMatch[1]) {
        throw new Error('Could not find intermediate usercontent URL in initial page.');
    }

    let intermediateUrl = intermediateUrlMatch[1].replace(/\\u003d/g, '=').replace(/\\u0026/g, '&');
    ...
    const newCookies = confirmResponse.headers['set-cookie'] ? cookies.concat(confirmResponse.headers['set-cookie']) : cookies;
    ...
    if (!form) {
        ...
        console.log("No confirmation form found, assuming direct download from intermediate URL.");
...
async function downloadChunk(url, cookies, chunkIndex, totalChunks, fileSize) {
    ...
    if (fs.existsSync(chunkFilePath)) {
        ...
        if (stats.size === expectedSize) {
            console.log(`Chunk ${chunkIndex}/${totalChunks - 1} already exists and is complete. Skipping.`);
            return;
        } else {
            console.log(`Chunk ${chunkIndex}/${totalChunks - 1} exists but is incomplete. Redownloading.`);
        }
    }
    ...
    console.log(`Downloading chunk ${chunkIndex}/${totalChunks - 1} (bytes ${start}-${end})...`);
